...
s.linkInternalFilters="javascript:,nykaa.com"
...
s.visitorNamespace="fsnecommerce"
s.trackingServer="fsnecommerce.sc.omtrdc.net"
s.trackingServerSecure="fsnecommerce.sc.omtrdc.net"
...
s.eVar99 = s.getLoadTime("browserapi","event1","event2");  
...
/* Time Parting */
s.eVar4 = s.getTimeParting('n','+5.5');
...
/*Brand Page View*/
if(_satellite.getVar('Events')=='Brand Page View')
...
var utmsource=s.Util.getQueryParam('utm_source');
var utmmedium=s.Util.getQueryParam('utm_medium');
var utmterm=s.Util.getQueryParam('utm_term');
var utmcontent=s.Util.getQueryParam('utm_content');
var utmcampaign=s.Util.getQueryParam('utm_campaign');

/*External Campaigns Tracking*/
if(!s.campaign)
	{
	if(utmsource==""&&utmmedium==""&&utmterm==""&&utmcontent==""&&utmcampaign=="")
		...
		{
s_camp=utmsource.toLowerCase()+':'+utmmedium.toLowerCase()+':'+utmcampaign.toLowerCase()+':'+utmcontent.toLowerCase()+':'+utmterm.toLowerCase(); 
		if(s_camp=='::::')
			{
				s.campaign="";
			}
...
/*Internal Promotions Tracking*/
if(!s.eVar22)
s.eVar22=s.Util.getQueryParam('intcmp');
s.eVar22=s.getValOnce(s.eVar22,'s_ev22',0);
if(s.eVar22)
s.events=s.apl(s.events,'event25',',',2);

/*Search location*/
if(s.events&&s.events.indexOf('event3,')>-1)
{
	s.linkTrackVars=s.apl(s.linkTrackVars,'eVar11',',',2);
	if(s.eVar2)
		s.eVar11=s.eVar2; // eVar2 holds the previous page name
}



var device = _satellite.browserInfo.deviceType;
if(device.toLowerCase() == "desktop")
s.eVar96 = device.toLowerCase();  
if (/Android|webOS|iPhone|iPad|iPod|BlackBerry|IEMobile|Opera Mini|Opera Mobile|Kindle|Windows Phone|PSP|AvantGo|Atomic Web Browser|Blazer|Chrome Mobile|Dolphin|Dolfin|Doris|GO Browser|Jasmine|MicroB|Mobile Firefox|Mobile Safari|Mobile Silk|Motorola Internet Browser|NetFront|NineSky|Nokia Web Browser|Obigo|Openwave Mobile Browser|Palm Pre web browser|Polaris|PS Vita browser|Puffin|QQbrowser|SEMC Browser|Skyfire|Tear|TeaShark|UC Browser|uZard Web|wOSBrowser|Yandex.Browser mobile/i.test(device))
s.eVar96 = "mobile"; 

/*Set Login Status*/
var login_status;  
// If Login event occurs, create a cookie and set the cookie value to Logged-In
if(s.events&&(s.events.indexOf('event26')>-1)) 
{ 
  login_status = "Logged-In";
  s.c_w("login_vi","Logged In"); 
}

//If Logout event occurs, delete cookie
if(s.events&&(s.events.indexOf('event27')>-1)) 
s.c_w("login_vi",'',-1);

//read cookie 
r=s.c_r("login_vi"); 
//If cookie is not set, set the value of eVar24 to "Not Logged In" , else set it to "Logged In"
if(r) 
login_status="Logged In";
else
...
s.eVar24= login_status;
  
/*Copy values to props*/
if(s.eVar1)
s.prop1="D=v1";
if(s.eVar2)
s.prop2="D=v2";
